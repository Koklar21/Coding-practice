...
VERSION: 2.3 (Financial Hardened / Memory Safe / UTC-Safe)
STATUS: PRODUCTION

CHANGELOG v2.3:
- Replaced naive datetime in VelocityGuard with UTC-aware timestamps.
- Monetary precision handled via Decimal.quantize instead of global context precision.
- model_snapshot added to audit log entries for explainability.
- Added logging on AI profile update failures.
"""

# Keep a sane global precision; do NOT confuse this with decimal places.
getcontext().prec = 28  # Standard high precision
    ...
    def detect_slow_boil(self, history: List[Decimal], current_amount: Decimal) -> bool:
        try:
            ...
            if len(history) < window - 1:
                return False
            # Look at last N - 1 from history plus current
            series = history[-(window - 1) :] + [current_amount]
            slope = self._linear_regression_slope(series)
            # Any positive slope => risk of gradual ramp-up
            return slope > Decimal("0.00")
        except Exception:
            return False
